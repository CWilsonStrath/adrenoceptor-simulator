...
import { Target, Lightbulb, Filter, ChevronLeft, ChevronRight } from 'lucide-react';
...
const PharmacologicalStrategy = ({ drugs, onStrategyChange }) => {
  ...
  React.useEffect(() => {
    if (targetReceptors.length === 0) {
      onStrategyChange({ matchingDrugs: drugs, criteria: null, drugType: null });
      return;
    }
  ...
  return (
    <div className="bg-gradient-to-br from-indigo-50 to-purple-50 rounded-lg border-2 border-indigo-200 p-2">
      ...
      <div className="flex justify-between items-center">
        ...
        {currentStep < 2 ? (
          <button
            ...
            disabled={currentStep === 0 && targetReceptors.length === 0}
            className={`flex items-center gap-1 px-2 py-1 rounded text-xs transition-all ${
              (currentStep === 0 && targetReceptors.length === 0)
                ? 'text-gray-400 cursor-not-allowed'
          ...
          <button
            ...
            className="px-2 py-1 rounded text-xs text-gray-600 hover:bg-gray-100 transition-all"
          >
            Start Over
          </button>
        )}
      ...
      {/* Results Summary - Always show if selections made */}
      {targetReceptors.length > 0 && (
        <div className="mt-2 p-2 bg-white rounded border border-indigo-200">
          ...
          <div className="text-xs text-gray-700 space-y-0.5">
            ...
            {currentStep >= 1 && <div>• Drug type: <span className="font-semibold">{drugType === 'any' ? 'Any' : drugType}</span></div>}
            {currentStep >= 2 && <div>• Selectivity: <span className="font-semibold">{selectivity === 'any' ? 'Any' : selectivity}</span></div>}
          </div>
        </div>
      )}
    </div>
